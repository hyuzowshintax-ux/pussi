        ...
        <div className="flex flex-wrap justify-center gap-3 mb-12">
          {Object.entries(reactionCounts).map(([emoji, count]) => (
            <button
              ...
              className="px-4 py-2.5 rounded-2xl glass-card border border-emerald-500/20 hover:border-emerald-400/50 hover:scale-110 active:scale-95 transition-all flex items-center space-x-2 text-white shadow-lg"
            >
              <span className="text-lg">{emoji}</span>
              <span className="font-mono text-xs font-bold text-emerald-300">+{count}</span>
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-5">
            <form
              ...
              className="glass-card rounded-3xl p-6 sm:p-8 border border-emerald-500/25 space-y-4 shadow-xl"
            >
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full bg-slate-900/80 border border-emerald-500/25 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-emerald-400 transition-colors"
                />
              </div>
                  ...
                  {["🔥", "🚀", "🧠", "⚜️", "❤️"].map((em) => (
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl text-base flex items-center justify-center transition-all ${
                        selectedReaction === em
                          ? "bg-emerald-600 border border-emerald-300 scale-110 shadow"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-900/80 border border-emerald-500/25 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-emerald-400 transition-colors resize-none"
                ></textarea>
              </div>

              <button
                type="submit"
                className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-semibold text-xs sm:text-sm flex items-center justify-center space-x-2 shadow-lg shadow-emerald-600/30 transition-all"
              >
          ...
          <div className="lg:col-span-7 space-y-4 max-h-[520px] overflow-y-auto pr-1">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className="glass-card rounded-2xl p-4 sm:p-5 border border-emerald-500/20 hover:border-emerald-400/40 transition-all space-y-2 shadow-md"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2.5">
                    <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-400 text-white flex items-center justify-center font-bold text-xs">
                      {msg.name.charAt(0)}
                    </div>
                    <div>
                      <h4 className="text-xs sm:text-sm font-bold text-white">{msg.name}</h4>
                      <span className="text-[11px] text-emerald-300 font-medium">{msg.role}</span>
                    </div>
                  </div>
