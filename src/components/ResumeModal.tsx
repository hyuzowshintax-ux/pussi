...
export const ResumeModal: React.FC<ResumeModalProps> = ({ isOpen, onClose, data }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-cloud-enter">
      <div className="relative w-full max-w-3xl glass-card rounded-3xl border border-purple-500/30 shadow-2xl shadow-purple-900/50 my-auto overflow-hidden">
        
        {/* Top Header Bar */}
        <div className="p-4 sm:p-5 bg-slate-900/90 border-b border-purple-500/20 flex items-center justify-between no-print">
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={handlePrint}
              className="px-3.5 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-semibold flex items-center space-x-1.5 shadow transition-all"
            >
        ...
        <div className="p-6 sm:p-10 space-y-8 max-h-[80vh] overflow-y-auto print:max-h-none print:p-0 text-slate-200">
          ...
          {/* Header Profile */}
          <div className="border-b border-purple-500/20 pb-6 space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              ...
              <span className="px-3 py-1 rounded-full text-xs font-mono font-bold bg-purple-500/20 text-purple-300 border border-purple-500/30 self-start sm:self-center">
                {data.profile.status}
              </span>
            </div>
          ...
          <div className="space-y-2">
            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-purple-400 flex items-center space-x-2">
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-mono font-bold uppercase tracking-wider text-pink-400 flex items-center space-x-2">
              <Award className="w-4 h-4" />
              <span>// Prestasi, Kepemimpinan & Organisasi</span>
            </h3>
            <div className="space-y-3">
              {data.timeline.experience.map((item, idx) => (
                <div key={idx} className="p-3.5 rounded-xl bg-slate-900/60 border border-purple-500/20 space-y-1">
                  <div className="flex justify-between items-center text-xs">
                    <strong className="text-white text-sm">{item.role}</strong>
                    <span className="font-mono text-purple-300 font-semibold">{item.period}</span>
                  </div>
                  <p className="text-xs text-purple-300 font-medium">{item.company} &bull; {item.location}</p>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {data.timeline.education.map((edu, idx) => (
                <div key={idx} className="p-3.5 rounded-xl bg-slate-900/60 border border-cyan-500/20 space-y-1">
                  <div className="flex justify-between items-center text-xs">
                    <strong className="text-white text-sm">{edu.institution}</strong>
                    <span className="font-mono text-cyan-300 font-semibold">{edu.period}</span>
                  </div>
                  <p className="text-xs text-cyan-300 font-medium">{edu.degree} &bull; {edu.location}</p>
