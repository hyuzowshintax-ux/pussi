...
4. Kamtibmas Guard (Portal Tanggap Disiplin & Edukasi Siswa)`
  ...
  return (
    <>
      ...
      <div className="fixed bottom-6 right-6 z-40">
        {!isOpen && (
          <button
            ...
            className="group relative flex items-center space-x-3 px-4 py-3.5 rounded-full bg-gradient-to-r from-purple-600 via-purple-700 to-pink-600 text-white shadow-2xl shadow-purple-600/40 hover:shadow-purple-500/60 hover:scale-105 transition-all duration-300 border border-white/20 animate-opening-pop"
      ...
      {/* Chat Drawer Box */}
      {isOpen && (
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[400px] h-[540px] max-h-[85vh] glass-card rounded-3xl border border-purple-500/30 shadow-2xl shadow-purple-900/40 flex flex-col overflow-hidden animate-opening-aperture">
          
          {/* Header */}
          <div className="p-4 bg-slate-900/80 border-b border-purple-500/20 flex items-center justify-between backdrop-blur-md">
            <div className="flex items-center space-x-3">
              <div className="relative">
                <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-purple-600 to-pink-600 flex items-center justify-center text-white shadow-md shadow-purple-600/30">
                  <Bot className="w-5 h-5" />
                </div>
                <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full bg-emerald-500 border-2 border-slate-900"></div>
              </div>
              <div>
                ...
                <p className="text-[11px] text-purple-300/80">Duta Intelegensia & AI Explorer</p>
              </div>
            </div>
                ...
                <div
                  className={`max-w-[78%] rounded-2xl p-3 shadow-md space-y-2 ${
                    msg.sender === "user"
                      ...
                      : "bg-slate-900/85 text-slate-200 border border-purple-500/20 rounded-tl-none leading-relaxed"
                  }`}
                >
          ...
          {/* Quick Prompts */}
          <div className="px-3 py-2 bg-slate-950/40 border-t border-purple-500/10 flex items-center gap-1.5 overflow-x-auto no-scrollbar">
            {quickPrompts.map((prompt, i) => (
              <button
                key={i}
                onClick={() => handleSend(prompt)}
                className="px-2.5 py-1 rounded-lg text-[11px] font-medium bg-slate-800/80 hover:bg-purple-900/50 hover:text-purple-300 text-slate-300 border border-purple-500/20 transition-all flex-shrink-0"
              >
                {prompt}
              </button>
            ))}
          ...
          >
            <input
              ...
              className="flex-1 bg-slate-800/80 border border-purple-500/30 rounded-xl px-3.5 py-2 text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-pink-500 transition-colors"
            />
            <button
              type="submit"
              disabled={!inputMessage.trim()}
              className="p-2 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white disabled:opacity-40 disabled:cursor-not-allowed hover:from-purple-500 hover:to-pink-500 transition-all shadow"
            >
